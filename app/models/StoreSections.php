...
class SectionView {
    ...
    public function generateSectionImageHTML_OLD($storeSection, $whereClause) {
    ...
    public function generateSectionImageHTML($storeSection, $whereClause) {
        ...
        // Extract the section images and section name
        $section_images = json_decode($section['section_image'], true);  // Assuming it's stored as JSON
        $section_name = $section['section_name'];  // Section name
        $section_id = $section['id'];  // Section ID
        
        // Generate the HTML output for all images
        $html = '<div class="container text-center">
        <p class="fw-bold fs-2">'.ucfirst($section_name).'</p>
        ';
        
        // Iterate through each image in the section
        foreach ($section_images as $index => $img_path) {
            $html .= '
            <div class="image-box mb-3">
                <p class="fs-3"> Box ' . ($index + 1) . '</p> <!-- Image index starts from 1 -->
                <img src="' . htmlspecialchars($img_path) . '" class="img-fluid" alt="' . htmlspecialchars($section_name) . '">
            </div>';
        }
